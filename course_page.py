...
if index.error == "":
    url = "https://vtopbeta.vit.ac.in/vtop/academics/common/StudentCoursePage"
    url = index.unified_session.post(url, headers=index.headers, verify=False)
    url = "https://vtopbeta.vit.ac.in/vtop/getCourseForCoursePage"
    ...
    if len(classId) > 1:
        ...
        for i in range(1):
            ...
            url = "https://vtopbeta.vit.ac.in/vtop/getSlotIdForCoursePage"
            ...
            if (len(table) > 2):
                for j in range(1, len(table)):
                    ...
                    button = button.split("'")
                    url = "https://vtopbeta.vit.ac.in/vtop/processViewStudentCourseDetail/?semSubId=" + button[
                    ...
                    if len(table1) > 0:
                        ...
                        for k in range(len(table1)):
                            ...
                            if len(link) > 0:
                                link = "https://vtopbeta.vit.ac.in" + link[0]['href']
                    ...
                    if len(a) > 0:
                        ...
                        for k in range(len(a)):
                            link = "https://vtopbeta.vit.ac.in" + a[k]['href']
                            link = index.unified_session.get(link, headers=index.headers, verify=False)
                            filename = link.headers['Content-disposition'].split(";")[1].split("=")[1]
                            filename = filename.replace("/", "+")
                            path1 = os.path.join(filepath, filename)
